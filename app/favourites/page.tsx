...
import { api, type EmployeeResult } from "@/services/api" // Added InvestorResult
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { FileText, Users, UserCircle2, Heart, X, ChevronDown, ChevronUp, Linkedin, MapPin } from "lucide-react" // Added Linkedin, MapPin
...
interface FavoriteEmployeeCardProps {
  ...
  isFavorite: boolean // Should always be true on this page
  isLoading: boolean
}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation()
                onToggleFavorite(employee) // This will effectively "unlike"
              }}
              disabled={isLoading}
              className={cn(
...
interface FavoriteEmployeeFundCardProps {
  ...
  onDislikeAction: (employee: EmployeeResult) => void
  onGenerateTemplate: (employee: EmployeeResult) => void
  isEmployeeFavorite: (employeeId: string) => boolean // To check if still favorite after an action
              ...
              <div className="max-h-[400px] overflow-y-auto">
                {employees.map((employee) => (
                  <FavoriteEmployeeCard
                    ...
                    isFavorite={isEmployeeFavorite(employee.id)} // Check current favorite status
...
export default function FavouritesPage() {
  const { favoriteInvestors, favoriteEmployees, loadFavorites, removeFromFavorites } = useApp()
  ...
  const handleInvestorToggleFavorite = useCallback(
    async (investorId: string) => {
      ...
      try {
        await api.updateInvestorSentiment(investorId, "dislike") // Unliking
  ...
  const handleEmployeeToggleFavorite = useCallback(
    async (employee: EmployeeResult) => {
      ...
      try {
        await api.updateEmployeeSentiment(employee.id, "dislike") // Unliking
  ...
  const handleNavigateToTemplate = (entityId: string, entityType: "investor" | "employee") => {
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {favoriteInvestors.map((investor) => (
                // CompactInvestorCard already handles its own template button via router.push
                <CompactInvestorCard
                  key={investor.id}
                  investor={investor}
            ...
            <div className="space-y-4">
              {Object.entries(employeesByFund).map(([fundName, employeesInFund]) => (
                <FavoriteEmployeeFundCard
                  ...
                  onGenerateTemplate={(employee) => handleNavigateToTemplate(employee.id, "employee")}
